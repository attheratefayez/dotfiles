...
vim.api.nvim_create_autocmd('PackChanged', {
  callback = function(ev)
    ...
    if name == 'LuaSnip' then
      if vim.fn.has 'win32' ~= 1 and vim.fn.executable 'make' == 1 then M.run_build(name, { 'make', 'install_jsregexp' }, ev.data.path) end
      return
    end
...
--because not every plugin should be loaded at startup
--example usage with vim.pack.add():
-- vim.pack.add({
